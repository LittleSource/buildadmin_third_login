							...
							>
								...
								>
									...
									>
										<el-radio
											...
											label="email"
											:disabled="
												!state.accountVerificationType.includes(
													'email'
												)
											"
											border
									...
									<el-row class="w100">
										...
										<el-col class="captcha-box" :span="8">
											<el-button
												size="large"
												@click="
													sendRegisterCaptcha(formRef)
												"
												:loading="
													state.sendCaptchaLoading
												"
												:disabled="
								...
								>
									<el-checkbox
										...
										:label="t('user.user.Remember me')"
										size="default"
									></el-checkbox>
									<div
										v-if="
											state.accountVerificationType
												.length > 0
										"
										@click="
								...
								<el-form-item class="form-buttons">
									...
									<el-button
										...
										size="large"
										>{{
											t("user.user.Back to login")
										}}</el-button
									>
									<el-button
										v-else
										@click="state.form.tab = 'register'"
										round
										plain
										type="info"
										size="large"
									>
										{{
											t(
												"user.user.No account yet? Click Register"
											)
										}}
									</el-button>
								</el-form-item>
			...
			<div class="retrieve-password-form">
				...
				>
					<el-form-item :label="t('user.user.Retrieval method')">
						<el-radio-group
							v-model="state.retrievePasswordForm.type"
						>
							<el-radio
								label="email"
								:disabled="
								...
								"
								border
								>{{ t("user.user.Via email") }}</el-radio
							>
							<el-radio
								label="mobile"
								:disabled="
					...
					<el-form-item
						prop="captcha"
						:label="t('user.user.Verification Code')"
					>
						<el-row class="w100">
							<el-col :span="16">
								<el-input
							...
							<el-col class="captcha-box" :span="8">
								<el-button
									@click="
										sendRetrieveCaptcha(retrieveFormRef)
									"
					...
					>
						<el-input
							...
							:placeholder="
								t('Please input field', {
									field: t('user.user.New password'),
								})
							"
							show-password
						>
					...
					<el-form-item>
						<el-button
							@click="state.showRetrievePasswordDialog = false"
							>{{ t("Cancel") }}</el-button
						>
						<el-button
							:loading="state.submitRetrieveLoading"
							@click="onSubmitRetrieve(retrieveFormRef)"
							type="primary"
							>{{ t("user.user.second") }}</el-button
						>
					</el-form-item>
				</el-form>
			</div>
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
	email: [
		buildValidatorData("required", t("user.user.mailbox")),
		buildValidatorData("email", t("user.user.mailbox")),
	],
	username: [
	...
	password: [
		buildValidatorData("required", t("user.user.password")),
		buildValidatorData("password"),
	],
	mobile: [
		buildValidatorData("required", t("user.user.mobile")),
		buildValidatorData("mobile"),
	],
...
const onSubmit = (
	formRef: InstanceType<typeof ElForm> | undefined = undefined
) => {
	...
	formRef!
		...
		.then((valid) => {
			if (valid) {
				...
				const func =
					state.form.registerType == "email" ? sendEms : sendSms;
